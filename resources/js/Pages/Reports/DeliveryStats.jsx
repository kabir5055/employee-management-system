...
export default function DeliveryStats({ auth, stats: initialStats, startDate, endDate }) {
    ...
    const validateDateRange = (start, end) => {
        ...
        if (start && end) {
            ...
            if (diffDays > 365) {
                errors.dateRange = 'Date range cannot exceed 1 year for delivery stats';
            }
        }
    ...
    const handleSearch = () => {
        try {
            ...
            // Make AJAX request
            fetch(`/api/reports/delivery-stats-data?${params.toString()}`)
    ...
    const totalDeliveries = filteredStats.reduce((sum, item) => sum + parseInt(item.total_deliveries || 0), 0);
    const totalAmount = filteredStats.reduce((sum, item) => sum + parseFloat(item.total_amount || 0), 0);
    const totalCommission = filteredStats.reduce((sum, item) => sum + parseFloat(item.total_commission || 0), 0);
    const averageCommissionRate = totalAmount > 0 ? (totalCommission / totalAmount) * 100 : 0;
    ...
    return (
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                                Analyze delivery performance and commission rates
                            </p>
                        </div>
                    </div>
        ...
        >
            <Head title="Delivery Statistics Report" />
                    ...
                    <div className="bg-white shadow overflow-hidden sm:rounded-md">
                        <div className="px-4 py-5 sm:px-6">
                            ...
                            <p className="mt-1 max-w-2xl text-sm text-gray-500">
                                Detailed delivery statistics for each employee
                            </p>
                        </div>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {isLoading ? (
                                        ...
                                        <tr>
                                            <td colSpan="7" className="px-6 py-4 text-center text-gray-500">
                                                No delivery stats found for the selected criteria
                                            </td>
                                        </tr>
                                    ) : (
                                        filteredStats.map((employee, index) => (
                                        <tr key={index} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {parseInt(employee.total_deliveries || 0).toLocaleString()}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">
                                                    {parseFloat(employee.commission_rate || 0).toFixed(2)}%
                                                </span>
                                            </td>
                                        </tr>
                                    ))
                        ...
                        {filteredStats.length === 0 && !isLoading && (
                            <div className="text-center py-12">
                                ...
                                <h3 className="mt-2 text-sm font-medium text-gray-900">No delivery data</h3>
                                <p className="mt-1 text-sm text-gray-500">
                                    No delivery statistics available for the selected date range.
                                </p>
                            </div>
                        )}
                    </div>
